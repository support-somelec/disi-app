...
export default function Register() {
  ...
  const needsDirection = ROLES_WITH_DIRECTION.includes(role);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError("");

    if (needsDirection && !directionId) {
        ...
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-8 shadow-2xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <select
                value={role}
                onChange={e => { setRole(e.target.value); setDirectionId(""); }}
                className="w-full px-4 py-2.5 bg-white/10 border border-white/20 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm"
              >
                {AVAILABLE_ROLES.map(r => (
                  <option key={r} value={r} className="bg-slate-800 text-white">{ROLE_LABELS[r] ?? r}</option>
                ))}
              </select>
            </div>
            ...
            {needsDirection && (
              <div>
                ...
                <select
                  ...
                  className="w-full px-4 py-2.5 bg-white/10 border border-white/20 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm"
                >
                  <option value="" className="bg-slate-800">— Choisir une direction —</option>
                  {directions?.map(d => (
                    <option key={d.id} value={d.id} className="bg-slate-800 text-white">{d.nom}</option>
                  ))}
                </select>
              </div>
            )}
