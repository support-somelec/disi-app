  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      if (event.target?.result) {
        setAttachments([...attachments, { file, base64: event.target.result.toString() }]);
      }
    };
          ...
          <AnimatePresence mode="wait">
            ...
            {step === 1 && (
              ...
              >
                ...
                <div className="pt-6 flex justify-end">
                  <Button type="submit" size="lg" isLoading={createPlanMutation.isPending} className="w-full sm:w-auto">
                      ...
                      {moyens.map((m, i) => (
                        <li key={i} className="p-4 flex justify-between items-center bg-white hover:bg-muted/20 transition-colors">
                          ...
                          <div className="font-semibold text-primary text-sm">
                            {Number(m.budget).toLocaleString('fr-MR')} MRU
                          </div>
                        </li>
                      ))}
                    </ul>
            ...
            {step === 3 && (
              <motion.div key="step3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8">
                ...
                <div className="border-2 border-dashed border-border hover:border-primary/50 transition-colors rounded-2xl p-10 text-center bg-muted/10 relative">
                  <input 
                    type="file" 
                  ...
                  <h3 className="font-semibold text-lg text-foreground">Cliquez ou glissez un fichier</h3>
                  <p className="text-sm text-muted-foreground mt-1">PDF, Excel, Images (Max 10MB)</p>
                </div>

                {attachments.length > 0 && (
                  <div className="space-y-3">
                    <h4 className="font-semibold text-foreground border-b pb-2">Fichiers à uploader</h4>
                    <ul className="space-y-2">
                      {attachments.map((att, i) => (
                        <li key={i} className="p-3 border rounded-xl flex items-center justify-between bg-white shadow-sm">
                          ...
                          <button onClick={() => setAttachments(attachments.filter((_, idx) => idx !== i))} className="p-2 text-muted-foreground hover:text-destructive transition-colors">
