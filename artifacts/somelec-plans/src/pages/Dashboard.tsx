...
import { FilePlus, Clock, CheckCircle2, ShieldCheck, Activity, Search, AlertCircle, TrendingDown, AlertTriangle } from "lucide-react";
...
export default function Dashboard() {
  ...
  const plans = allPlans.filter(p => {
    const matchSearch = !search || p.titre.toLowerCase().includes(search.toLowerCase()) || (p.directionNom ?? "").toLowerCase().includes(search.toLowerCase()) || (p.reference ?? "").toLowerCase().includes(search.toLowerCase());
  ...
  const stats = {
    ...
    pendingDGA: allPlans.filter(p => p.statut === "en_attente_dga").length,
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        {seesAll && (
          <Card>
            <CardContent className="p-5">
              <div className="flex justify-between items-start">
                <div><p className="text-xs font-medium text-muted-foreground mb-1">En attente</p><h3 className="text-3xl font-bold text-warning">{stats.pendingCT + stats.pendingDGA + stats.pendingDG}</h3></div>
        ...
        {overBudgetPlans.length > 0 && (
          <Card className="bg-destructive/5 border-destructive/20">
            <CardContent className="p-5">
              <div className="flex justify-between items-start">
                <div><p className="text-xs font-medium text-muted-foreground mb-1">Dépassement</p><h3 className="text-3xl font-bold text-destructive">{overBudgetPlans.length}</h3></div>
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-border/50">
              {plans.length === 0 ? (
                ...
                return (
                  <tr key={plan.id} className={`hover:bg-muted/30 transition-colors group ${isOverBudget ? "bg-destructive/5" : ""}`}>
                    ...
                    <td className="px-5 py-4">
                      <div className={`font-semibold group-hover:text-primary transition-colors flex items-center gap-2 ${isOverBudget ? "text-destructive" : "text-foreground"}`}>
                        {isOverBudget && <AlertTriangle className="w-4 h-4 shrink-0" />}
                        {plan.titre}
                      </div>
                    </td>
                      ...
                      <td className="px-5 py-4 text-right">
                        ...
                        {(plan.montantConsomme ?? 0) > 0 && (
                          <div className={`text-xs font-medium ${isOverBudget ? "text-destructive font-bold" : "text-warning"}`}>
                            {isOverBudget && "⚠ "}{formatCurrency(plan.montantConsomme ?? 0)} consommé
                          </div>
                        )}
                      </td>
                    )}
